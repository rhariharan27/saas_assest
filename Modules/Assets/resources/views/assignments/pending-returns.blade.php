...
@section('title', __('Pending Return Requests'))
...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="row mb-4">
            <div class="col-12">
                <h4 class="py-3 mb-0">
                    <span class="text-muted fw-light">{{ __('Asset Management') }} /</span> {{ __('Pending Return Requests') }}
                </h4>
            </div>
        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="d-flex flex-column">
                                <div class="card-title mb-auto">
                                    ...
                                    <p class="text-muted">{{ __('Return Requests') }}</p>
                                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div class="d-flex flex-column">
                                <div class="card-title mb-auto">
                                    <h5 class="mb-1 text-nowrap">{{ __('Urgent') }}</h5>
                                    <p class="text-muted">{{ __('Older than 3 Days') }}</p>
                                </div>
                                <div class="chart-statistics">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div class="d-flex flex-column">
                                <div class="card-title mb-auto">
                                    <h5 class="mb-1 text-nowrap">{{ __('Today') }}</h5>
                                    <p class="text-muted">{{ __('New Requests') }}</p>
                                </div>
                                <div class="chart-statistics">
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="d-flex flex-column">
                                <div class="card-title mb-auto">
                                    ...
                                    <p class="text-muted">{{ __('Time in Hours') }}</p>
                                </div>
                                <div class="chart-statistics">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <div class="row gy-3">
                    ...
                    <div class="col-md-3">
                        <label for="filter_priority" class="form-label">{{ __('Status') }}</label>
                        <select id="filter_priority" class="form-select">
                            <option value="">{{ __('All Assignments') }}</option>
                            <option value="urgent">{{ __('Urgent (3+ days)') }}</option>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">{{ __('Pending Return Requests') }}</h5>
                <div>
                    <button type="button" class="btn btn-success btn-sm" id="bulkApproveBtn" disabled>
                        <i class="bx bx-check me-1"></i>{{ __('Bulk Approve') }}
                    </button>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ __('Return Request Details') }}</h5>
        ...
        <div class="modal fade" id="returnResponseModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ __('Respond to Return Request') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="returnResponseForm" onsubmit="return false;">
                        ...
                        <div class="modal-body">
                            <div class="text-center mb-4">
                                ...
                                <h6 class="mb-1" id="responseEmployeeName">{{ __('Employee Name') }}</h6>
                                <p class="text-muted mb-0" id="responseAssetInfo">{{ __('Asset Information') }}</p>
                            </div>

                            <div class="alert alert-info">
                                <h6 class="alert-heading mb-2">{{ __('Return Reason:') }}</h6>
                            ...
                            <div class="row">
                                        ...
                                        <div class="col-6">
                                            <div class="form-check">
                                                ...
                                                <label class="form-check-label text-success" for="approveReturn">
                                                    <i class="bx bx-check-circle me-1"></i>{{ __('Approve Return') }}
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div class="form-check">
                                                ...
                                                <label class="form-check-label text-danger" for="rejectReturn">
                                                    <i class="bx bx-x-circle me-1"></i>{{ __('Reject Return') }}
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="col-12 mb-3">
                                    <label for="responseNotes" class="form-label">{{ __('Admin Notes') }}</label>
                                    <textarea class="form-control" id="responseNotes" name="notes" rows="3" placeholder="{{ __('Optional notes about your decision...') }}"></textarea>
                                </div>
                            </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary" id="submitResponseBtn">{{ __('Submit Response') }}</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal fade" id="bulkApproveModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ __('Bulk Approve Returns') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="text-center mb-4">
                            ...
                            <h6 class="mb-1">{{ __('Approve Multiple Returns') }}</h6>
                            <p class="text-muted mb-0">{{ __('You are about to approve') }} <span id="bulkCount">0</span> {{ __('return requests') }}</p>
                        </div>

                        <div class="alert alert-warning">
                            <i class="bx bx-info-circle me-2"></i>
                            {{ __('This action will approve all selected return requests. Employees will be notified and can proceed with returning their assets.') }}
                        </div>

                        <div class="form-group">
                            <label for="bulkNotes" class="form-label">{{ __('Bulk Approval Notes (Optional)') }}</label>
                            <textarea class="form-control" id="bulkNotes" rows="2" placeholder="{{ __('Notes to be added to all approved requests...') }}"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                        <button type="button" class="btn btn-success" id="confirmBulkApprove">
                            <i class="bx bx-check me-1"></i>{{ __('Approve All Selected') }}
                        </button>
                    </div>
                </div>
            </div>
