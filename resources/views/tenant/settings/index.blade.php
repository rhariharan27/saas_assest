                ...
                <ul class="nav nav-align-left nav-pills flex-column" id="settingsMenu">
                    ...
                    <li class="nav-item mb-1">
                        <a class="nav-link" href="?tab=employeeSettings" data-bs-toggle="pill"
                            ...
                            <span class="align-middle">{{ __('Employee Settings') }}</span>
                        </a>
                    </li>
                    @if ($addonService->isAddonEnabled(ModuleConstants::PAYROLL))
                        <li class="nav-item mb-1">
                            <a class="nav-link" href="?tab=payrollSettings" data-bs-toggle="pill"
                                ...
                                <span class="align-middle">{{ __('Payroll Settings') }}</span>
                            </a>
                        </li>
                    @endif
                    <li class="nav-item mb-1">
                    ...
                    @if ($addonService->isAddonEnabled(ModuleConstants::AI_CHATBOT))
                        <li class="nav-item mb-1">
                            <a class="nav-link" href="?tab=aiSettings" data-bs-toggle="pill" data-bs-target="#aiSettings">
                                <i class="bx bx-brain bx-sm me-1_5"></i> {{ __('AI Settings') }} <span
                                    class="badge bg-danger rounded-pill ms-auto">{{ __('Beta') }}</span>
                            </a>
                        </li>
                    @endif
                </ul>
            ...
            <div class="tab-content p-0">
                    ...
                    <form action="{{ route('settings.updateAppSettings') }}" method="POST">
                        ...
                        <div class="card mb-6">
                            ...
                            <div class="card-body">
                                <div class="row g-6">
                                    <div class="col-12 col-md-6">
                                        <label for="mAppVersion" class="form-label">{{ __('Mobile App Version') }}</label>
                                        <input type="text" class="form-control" id="mAppVersion" name="mAppVersion"
                                            value="{{ $settings->m_app_version ?? '' }}">
                                    </div>
                                    <div class="col-12 col-md-6">
                                        <label for="locationDistanceFilter" class="form-label">{{ __('Location Distance Filter') }} ({{ __('Kilometers') }})</label>
                                    ...
                                    @if ($settings->is_helper_text_enabled)
                                        <div class="alert alert-primary alert-dismissible" role="alert">
                                            <h6 class="alert-heading">{{ __('Important Note:') }} </h6>
                                            <p class="mb-0">{{ __('Please note that the location distance filter is used to filter out location updates that are less than the specified distance. This is useful to reduce the number of location updates sent to the server.') }}</p>
                                            <p>{{ __('We recommend using a 10') }} {{ __('Kilometers') }} {{ __('distance filter for most use cases.') }}
                                            </p>
                                        </div>
                                    @endif
                                </div>
                ...
                <div class="tab-pane fade" id="employeeSettings" role="tabpanel">
                    <form action="{{ route('settings.updateEmployeeSettings') }}" method="POST">
                        ...
                        <div class="card mb-6">
                            <div class="card-header">
                                <h5 class="card-title m-0">{{ __('Employee Settings') }}</h5>
                            </div>
                            <div class="card-body">
                                <div class="row g-6">
                                    ...
                                    <!-- Device Verification -->
                                    <div class="col-12 col-md-6">
                                        <label class="form-label">{{ __('Enable Device Verification') }}</label>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox"
                                            ...
                                            <label class="form-check-label" for="isDeviceVerificationEnabled">
                                                {{ $settings->is_device_verification_enabled ? __('Enabled') : __('Disabled') }}</label>
                                        </div>
                                    </div>
                ...
                <div class="tab-pane fade" id="trackingSettings" role="tabpanel">
                    <form action="{{ route('settings.updateTrackingSettings') }}" method="POST">
                        ...
                        <div class="card mb-6">
                            <div class="card-header">
                                <h5 class="card-title m-0">{{ __('Tracking Settings') }}</h5>
                            </div>
                            <div class="card-body">
                                <div class="row g-6">
                                    <!-- Offline Check Time -->
                                    <div class="col-12 col-md-6">
                                        <label for="offlineCheckTime" class="form-label">{{ __('Offline Check Time') }} ({{ __('Seconds') }})</label>
                ...
                <div class="tab-pane fade" id="codePrefixSettings" role="tabpanel">
                    <form action="" method="POST">
                        ...
                        <div class="card mb-6">
                            <div class="card-header">
                                <h5 class="card-title m-0">{{ __('Code Prefix/Suffix') }}</h5>
                            </div>
                            <div class="card-body">
                                <div class="row g-6">
                ...
                <div class="tab-pane fade" id="mapsSettings" role="tabpanel">
                    <form action="{{ route('settings.updateMapSettings') }}" method="POST">
                        ...
                        <div class="card mb-6">
                            <div class="card-header">
                                <h5 class="card-title m-0">{{ __('Maps Settings') }}</h5>
                            </div>
                            <div class="card-body">
                                <div class="row g-6">
                                    <!-- Map Provider -->
                ...
                <div class="tab-pane fade" id="companySettings">
                    <form action="{{ route('settings.updateCompanySettings') }}" method="POST"
                        ...
                        <div class="card mb-6">
                            <div class="card-header">
                                <h5 class="card-title m-0">{{ __('Company Settings') }}</h5>
                            </div>
                            <div class="card-body">
                                <div class="row g-6">
                                    ...
                                    <div class="col-12 col-md-12">
                                        ...
                                        <small class="text-muted mt-2 d-block">{{ __('Click on the logo to change it. Allowed formats: JPG, PNG, Max size: 2MB') }}</small>
                                    </div>
                                    <!-- Company Name -->
                                    <div class="col-12 col-md-6">
                ...
                @if ($addonService->isAddonEnabled(ModuleConstants::AI_CHATBOT))
                    ...
                    <div class="tab-pane fade" id="aiSettings" role="tabpanel">
                        <form action="{{ route('settings.updateAiSettings') }}" method="POST">
                            ...
                            <div class="card mb-6">
                                <div class="card-header">
                                    <h5 class="card-title m-0">{{ __('AI Settings') }} <span
                                            class="badge bg-danger rounded-pill ms-auto">{{ __('Beta') }}</span>
                                    </h5>
                                </div>
                                <div class="card-body">
                                    <div class="row g-6">
                                        ...
                                        <div class="col-12">
                                            <label class="form-label" for="chatGptKey">{{ __('Chat GPT API Key') }}</label>
                                            <input type="text" class="form-control" id="chatGptKey"
                                                name="chat_gpt_key"
                                                value="{{ old('chat_gpt_key', $settings->chat_gpt_key) }}"
                                                maxlength="500">
                                            <small class="form-text text-muted">{{ __('Enter the API key to enable Chat GPT integrations.') }}</small>
                                        </div>

                                        <!-- Enable AI Chat Globally -->
                                        <div class="col-12">
                                            <label class="form-label">{{ __('Enable AI Chat Globally') }}</label>
                                            <div class="form-check form-switch">
                                                ...
                                                <label class="form-check-label" for="enableAiChatGlobal">
                                                    {{ $settings->enable_ai_chat_global ? __('Enabled') : __('Disabled') }}
                                                </label>
                                            </div>
                                            <small class="form-text text-muted">{{ __('When disabled, all AI features will be turned off globally.') }}</small>
                                        </div>
                                        ...
                                        <div id="aiOptions" class="col-12"
                                            ...
                                            <div class="row g-6">
                                                <!-- Enable AI for Admin -->
                                                <div class="col-12 col-md-6">
                                                    <label class="form-label">{{ __('Enable AI for Admin') }}</label>
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox"
                                                        ...
                                                        <label class="form-check-label" for="enableAiForAdmin">
                                                            {{ $settings->enable_ai_for_admin ? __('Enabled') : __('Disabled') }}
                                                        </label>
                                                    </div>
                                                    <small class="form-text text-muted">{{ __('Allow administrators to access AI features.') }}</small>
                                                </div>

                                                <!-- Enable AI for Employee Self Service -->
                                                <div class="col-12 col-md-6">
                                                    <label class="form-label">{{ __('Enable AI for Employee Self Service') }}</label>
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox"
                                                        ...
                                                        <label class="form-check-label"
                                                            ...
                                                            {{ $settings->enable_ai_for_employee_self_service ? __('Enabled') : __('Disabled') }}
                                                        </label>
                                                    </div>
                                                    <small class="form-text text-muted">{{ __('Enable AI features for employee self-service in mobile app.') }}</small>
                                                </div>

                                                <!-- Enable AI for Business Intelligence -->
                                                <div class="col-12 col-md-6">
                                                    <label class="form-label">{{ __('Enable AI for Business Intelligence') }}</label>
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox"
                                                        ...
                                                        <label class="form-check-label"
                                                            ...
                                                            {{ $settings->enable_ai_for_business_intelligence ? __('Enabled') : __('Disabled') }}
                                                        </label>
                                                    </div>
                                                    <small class="form-text text-muted">{{ __('Enable AI for advanced business intelligence insights.') }}</small>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    @if ($settings->is_helper_text_enabled)
                                        <div class="alert alert-primary alert-dismissible mt-3" role="alert">
                                            <h6 class="text-primary mb-4 fw-bold">{{ __('What are AI Settings?') }}</h6>
                                            <p class="mb-0">
                                                {{ __('AI settings allow you to enable or disable AI-powered features across') }} <strong>{{ config('variables.templateFullName') }}.</strong> 
                                                {{ __('These include Chat GPT integrations for generating intelligent responses, enhancing workflows, and providing insights. You can manage access and enable AI features for specific use cases such as employee self-service, administrative tools, and business intelligence.') }}
                                            </p>
                                            <ul class="mt-3">
                                                <li>
                                                    <strong>{{ __('Enable AI for Admin:') }}</strong> {{ __('Empower administrators with AI features to automate repetitive tasks, generate reports, and streamline HR operations efficiently.') }}
                                                </li>
                                                <li>
                                                    <strong>{{ __('Enable AI for Employee Self Service:') }}</strong> {{ __('AI capabilities for employees are accessible through mobile app, designed to cater to both field and office employees. Features include:') }}
                                                    <ul>
                                                        <li>{{ __('Smart chatbots for answering HR-related queries (e.g., leave balance, payroll details).') }}</li>
                             ...
                             <li>Streamlined processes for requesting leave, reimbursements, and other tasks.</li> --}}
                                                ...
                                                <li>
                                                    <strong>{{ __('Enable AI for Business Intelligence:') }}</strong> {{ __('Use AI to generate actionable insights and analytics for your organization. This feature aids in decision-making by analyzing HR trends, employee performance, and workforce planning.') }}
                                                </li>
                                            </ul>
                                    ...
                                    <div class="alert alert-warning alert-dismissible mt-3" role="alert">
                                        <h6 class="text-warning mb-4 fw-bold">{{ __('Important Note:') }}</h6>
                                        <p class="mb-0">
                                            {{ __('AI features are currently in beta and may produce unexpected results. We recommend using AI cautiously and monitoring its output for accuracy.') }}
                                            <br><br>
                                            {{ __('Currently, the AI options are only available for Admin and Business Intelligence purposes. Features for Employee Self-Service will be introduced in future updates. Admins can use AI to automate HR processes, generate intelligent reports, and enhance decision-making through data-driven insights. Business Intelligence users can leverage AI to analyze trends, identify patterns, and forecast workforce needs.') }}
                                        </p>
                                        <button type="button" class="btn-close" data-bs-dismiss="alert"
                                            aria-label="Close"></button>
                                    </div>
                ...
                @if ($addonService->isAddonEnabled(ModuleConstants::PAYROLL))
                    ...
                    <div class="tab-pane fade" id="payrollSettings" role="tabpanel">
                        <form action="{{ route('settings.updatePayrollSettings') }}" method="POST">
                            ...
                            <div class="card mb-6">
                                <div class="card-header">
                                    <h5 class="card-title m-0">{{ __('Payroll Settings') }}</h5>
                                </div>
                                <div class="card-body">
                                    <div class="row g-6">
                                        <!-- Payroll Frequency -->
                                        <div class="col-12 col-md-6">
                                            <label for="payrollFrequency" class="form-label">{{ __('Payroll Frequency') }}</label>
                                        ...
                                        <!-- Payroll Start Date -->
                                        <div class="col-12 col-md-6">
                                            <label for="payrollStartDate" class="form-label">{{ __('Payroll Start Date') }} ({{ __('Date of Month') }})</label>
                                        ...
                                        <div class="col-12 col-md-6">
                                            <label for="payrollCutoffDate" class="form-label">{{ __('Payroll Cut-Off Date') }} ({{ __('Date of Month') }})</label>
                                        ...
                                        <div class="col-12 col-md-6">
                                            <label for="autoPayrollProcessing" class="form-label">{{ __('Enable Automatic Payroll Processing') }}</label>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox"
                                                    id="autoPayrollProcessing" name="autoPayrollProcessing"
                                                    {{ $settings->auto_payroll_processing ?? false ? 'checked' : '' }}>
                                                <label class="form-check-label" for="autoPayrollProcessing">{{ __('Enable') }}</label>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="card-footer text-end">
                        ...
                        <div class="card mt-4">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h5 class="mb-0"><i class="bx bx-adjust text-muted"></i> {{ __('Payroll Adjustments') }}</h5>
                                <button class="btn btn-sm btn-primary" data-bs-toggle="offcanvas"
                                    ...
                                    <i class="bx bx-plus"></i> {{ __('Add Adjustment') }}
                                </button>
                            </div>
                            <div class="card-body">
...
@section('page-script')
    ...
    <script>
        // Initialize translations for dynamic language support
        window.translations = {
            // General Settings
            appName: '{{ __("App Name") }}',
            country: '{{ __("Country") }}',
            phoneCountryCode: '{{ __("Phone Country Code") }}',
            currency: '{{ __("Currency") }}',
            currencySymbol: '{{ __("Currency Symbol") }}',
            distanceUnit: '{{ __("Distance Unit") }}',
            kilometers: '{{ __("Kilometers") }}',
            miles: '{{ __("Miles") }}',
            enableHelperText: '{{ __("Enable Helper Text") }}',
            enabled: '{{ __("Enabled") }}',
            disabled: '{{ __("Disabled") }}',
            saveChanges: '{{ __("Save Changes") }}',
            
            // App Settings
            mobileAppVersion: '{{ __("Mobile App Version") }}',
            locationDistanceFilter: '{{ __("Location Distance Filter") }}',
            
            // Employee Settings
            enableDeviceVerification: '{{ __("Enable Device Verification") }}',
            defaultPassword: '{{ __("Default Password") }}',
            
            // Tracking Settings
            trackingSettings: '{{ __("Tracking Settings") }}',
            offlineCheckTime: '{{ __("Offline Check Time") }}',
            seconds: '{{ __("Seconds") }}',
            
            // Code Prefix Settings
            codePrefixSuffix: '{{ __("Code Prefix/Suffix") }}',
            employeeCodePrefix: '{{ __("Employee Code Prefix") }}',
            orderPrefix: '{{ __("Order Prefix") }}',
            
            // Maps Settings
            mapsSettings: '{{ __("Maps Settings") }}',
            mapProvider: '{{ __("Map Provider") }}',
            google: '{{ __("Google") }}',
            mapZoomLevel: '{{ __("Map Zoom Level") }}',
            centerLatitude: '{{ __("Center Latitude") }}',
            centerLongitude: '{{ __("Center Longitude") }}',
            mapApiKey: '{{ __("Map API Key") }}',
            
            // Company Settings
            companySettings: '{{ __("Company Settings") }}',
            companyLogo: '{{ __("Company Logo") }}',
            companyName: '{{ __("Company Name") }}',
            companyEmail: '{{ __("Company Email") }}',
            companyPhone: '{{ __("Company Phone") }}',
            companyWebsite: '{{ __("Company Website") }}',
            address: '{{ __("Address") }}',
            city: '{{ __("City") }}',
            state: '{{ __("State") }}',
            zipcode: '{{ __("Zipcode") }}',
            registrationNumber: '{{ __("Registration Number") }}',
            companyTaxId: '{{ __("Company Tax ID") }}',
            
            // AI Settings
            aiSettings: '{{ __("AI Settings") }}',
            beta: '{{ __("Beta") }}',
            chatGptApiKey: '{{ __("Chat GPT API Key") }}',
            enableAiChatGlobally: '{{ __("Enable AI Chat Globally") }}',
            enableAiForAdmin: '{{ __("Enable AI for Admin") }}',
            enableAiForEmployeeSelfService: '{{ __("Enable AI for Employee Self Service") }}',
            enableAiForBusinessIntelligence: '{{ __("Enable AI for Business Intelligence") }}',
            
            // Payroll Settings
            payrollSettings: '{{ __("Payroll Settings") }}',
            payrollFrequency: '{{ __("Payroll Frequency") }}',
            monthly: '{{ __("Monthly") }}',
            weekly: '{{ __("Weekly") }}',
            biWeekly: '{{ __("Bi-Weekly") }}',
            daily: '{{ __("Daily") }}',
            payrollStartDate: '{{ __("Payroll Start Date") }}',
            payrollCutOffDate: '{{ __("Payroll Cut-Off Date") }}',
            enableAutomaticPayrollProcessing: '{{ __("Enable Automatic Payroll Processing") }}',
            enable: '{{ __("Enable") }}',
            payrollAdjustments: '{{ __("Payroll Adjustments") }}',
            addAdjustment: '{{ __("Add Adjustment") }}',
            name: '{{ __("Name") }}',
            code: '{{ __("Code") }}',
            type: '{{ __("Type") }}',
            applicability: '{{ __("Applicability") }}',
            amount: '{{ __("Amount") }}',
            percentage: '{{ __("Percentage") }}',
            benefit: '{{ __("Benefit") }}',
            deduction: '{{ __("Deduction") }}',
            global: '{{ __("Global") }}',
            employeeSpecific: '{{ __("Employee-Specific") }}',
            actions: '{{ __("Actions") }}',
            edit: '{{ __("Edit") }}',
            delete: '{{ __("Delete") }}',
            updateAdjustment: '{{ __("Update Adjustment") }}'
        };
