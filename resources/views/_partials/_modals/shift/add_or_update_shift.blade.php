  ...
  <div class="offcanvas-body mx-0 flex-grow-0">
    <form class="add-edit-shift-form pt-0" id="shiftForm" onsubmit="return false;">
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" id="shiftName" placeholder="{{ __("e.g., General Shift, Night Shift") }}"
               name="name" required />
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" id="shiftCode" placeholder="{{ __("Unique Code (e.g., GS01)") }}" name="code"
               required />
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <input type="text" class="form-control flatpickr-input" placeholder="{{ __("HH:MM") }}" id="startTime" name="start_time"
                 required readonly="readonly" />
          <div class="invalid-feedback"></div>
        </div>
        <div class="col-md-6">
          <label for="endTime" class="form-label">{{ __("End Time") }} <span class="text-danger">*</span></label>
          <input type="text" class="form-control flatpickr-input" placeholder="{{ __("HH:MM") }}" id="endTime" name="end_time"
                 required readonly="readonly" />
          <div class="invalid-feedback"></div>
        </div>
      </div>
      ...
      {{-- Shift Days --}}
      <div class="mb-3">
        <label class="form-label d-block">{{ __("Working Days") }}</label>
        <div class="d-flex flex-wrap justify-content-start gap-3">
          @foreach(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $day)
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="1" id="{{ $day }}Toggle" name="{{ $day }}">
              <label class="form-check-label" for="{{ $day }}Toggle"> {{ __(ucfirst($day)) }} </label>
            </div>
          @endforeach
      ...
      <div class="mb-3">
        ...
        <textarea class="form-control" id="shiftNotes" name="notes" rows="3" placeholder="{{ __("Optional notes...") }}"></textarea>
        <div class="invalid-feedback"></div>
      </div>
      ...
      <div class="mt-4 text-end">
        <button type="reset" class="btn btn-label-secondary me-2" data-bs-dismiss="offcanvas">{{ __("Cancel") }}</button>
        <button type="submit" class="btn btn-primary data-submit" id="submitShiftBtn">{{ __("Submit") }}</button>
      </div>
    </form>
  </div>
</div>
